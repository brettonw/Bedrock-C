...
#define     TEST_XYAPPROX(_x, _y, _e) { bool result_xyop = abs((_x) - (_y)) <= _e; (result_xyop ? (Log::debug () << "    PASS") : (Log::error () << "    FAIL")) << ": ((" #_x ") ~= (" #_y ")), (" << (_x) << " ~= " << (_y) << "), (delta = " << abs ((_x) - (_y)) << " < " << (_e) << ")" << endl; TEST_ASSERTION(result_xyop); }
