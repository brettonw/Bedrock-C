...
class BoundingBall {
    ...
    private:
        ...
        //   https://people.inf.ethz.ch/gaertner/subdir/texts/own_work/esa99_final.pdf
        static Vector q0;
        static Point  c[dimension + 1];
        static Vector v[dimension + 1];
        static Vector a[dimension + 1];
        static Scalar z[dimension + 1];
        static Scalar f[dimension + 1];
        static Scalar r[dimension + 1];
        static Scalar currentSquaredRadius;
        static Point  currentCenter;
        static uint   fsize;

#define CD(x) static_cast<Coordinate> (x)
        static bool addBoundaryPoint (const Point& boundaryPoint) {
            Scalar eps = square<Scalar>(Point::getEpsilon());
            ...
            if (fsize == 0) {
                ...
                */
                ...
                for (uint i = 0; i < dimension; ++i) {
                    e += square<Scalar> (boundaryPoint[CD(i)] - c[fsize-1][CD(i)]);
                }
        ...
        static BoundingBall fromBoundaryPoints (const Point* boundaryPoints, uint boundaryPointCount) {
            bool success = true;
            for (int i = 0; success and (i < boundaryPointCount); ++i) {
                fsize = i;
                success = addBoundaryPoint (boundaryPoints[i]);
            }
            if (success) {
...
template<typename Scalar, uint dimension>
typename BoundingBall<Scalar, dimension>::Vector BoundingBall<Scalar, dimension>::q0;

template<typename Scalar, uint dimension>
typename BoundingBall<Scalar, dimension>::Point  BoundingBall<Scalar, dimension>::c[dimension + 1];

template<typename Scalar, uint dimension>
typename BoundingBall<Scalar, dimension>::Vector BoundingBall<Scalar, dimension>::v[dimension + 1];

template<typename Scalar, uint dimension>
typename BoundingBall<Scalar, dimension>::Vector BoundingBall<Scalar, dimension>::a[dimension + 1];

template<typename Scalar, uint dimension>
Scalar BoundingBall<Scalar, dimension>::z[dimension + 1];

template<typename Scalar, uint dimension>
Scalar BoundingBall<Scalar, dimension>::f[dimension + 1];

template<typename Scalar, uint dimension>
Scalar BoundingBall<Scalar, dimension>::r[dimension + 1];

template<typename Scalar, uint dimension>
Scalar BoundingBall<Scalar, dimension>::currentSquaredRadius;

template<typename Scalar, uint dimension>
typename BoundingBall<Scalar, dimension>::Point  BoundingBall<Scalar, dimension>::currentCenter;

template<typename Scalar, uint dimension>
uint   BoundingBall<Scalar, dimension>::fsize;
