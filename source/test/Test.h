...
#define     TEST_XYF(_x, _y, _p) { bool result_xyop = abs((_x) - (_y)) <= _p; (result_xyop ? (Log::debug () << "    PASS") : (Log::error () << "    FAIL")) << ": ((" #_x ") ~= (" #_y ")), ((" << (_x) << " - " << (_y) << ") <= " << (_p) << "), (" << abs ((_x) - (_y)) << " <= " << (_p) << ")" << endl; TEST_ASSERTION(result_xyop); }
