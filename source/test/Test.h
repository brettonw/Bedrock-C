...
#define     TEST_XYU(_x, _y) { bool result_xyop = ulpEquals((_x), (_y)); (result_xyop ? (Log::debug () << "    PASS") : (Log::error () << "    FAIL")) << ": ((" #_x ") ~= (" #_y ")), (" << (_x) << " ~= " << (_y) << "), (delta = " << abs ((_x) - (_y)) << ")" << endl; TEST_ASSERTION(result_xyop); }
